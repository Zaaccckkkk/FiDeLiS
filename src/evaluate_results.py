...
def eval_result(predict_file, cal_f1=True, topk = -1):
    ...
    for item in ["prediction_llm", "prediction_direct_answer"]:
        ...
        recall_list = []
        error_list = []
        with open(predict_file, 'r') as f, open(detailed_eval_file, 'a') as f2, open(error_file, 'a') as f3:
            for line in f:
                try:
        ...
        if len(acc_list) == 0:
            result_str = str(item) + " No predictions to evaluate."
            result = {
                "Accuracy": 0,
                "Hit": 0,
                "F1": 0,
