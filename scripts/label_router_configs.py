...
def normalize_for_path_match(value):
    ...
    text = re.sub(r"\+([0-9]{3,4})-([0-9]{2})-([0-9]{2})t00:00:00z", r"\1 \2 \3", text)
    text = re.sub(r"[^a-z0-9]+", " ", text)
    return re.sub(r"\s+", " ", text).strip()


def parse_crlt_gold_triples(triples_text):
    triples = []
    text = str(triples_text or "")
...
def load_crlt_qa_gold_triples(crlt_data_dir):
    gold_file = os.path.join(crlt_data_dir, "CR-LT-QA.json")
    with open(gold_file, "r", encoding="utf-8") as f:
        rows = json.load(f)
    return {
        row["id"]: parse_crlt_gold_triples(row.get("KG Triples"))
        for row in rows
        if row.get("id") is not None
    }


def score_crlt_path_coverage(reasoning_paths, gold_triples):
    path_text = normalize_for_path_match(" ".join(str(path) for path in reasoning_paths or []))
    matched = []
    missing = []
    for triple in gold_triples or []:
        subject = normalize_for_path_match(triple.get("subject", ""))
        relation = normalize_for_path_match(triple.get("relation", ""))
        object_value = normalize_for_path_match(triple.get("object", ""))
        is_match = (
            bool(subject)
            and bool(relation)
            and bool(object_value)
            and subject in path_text
            and relation in path_text
            and object_value in path_text
        )
        if is_match:
    ...
    total = len(gold_triples or [])
    matched_count = len(matched)
    return {
        "matched_count": matched_count,
        "total_count": total,
        "coverage": matched_count / total if total else 0.0,
        "full_coverage": total > 0 and matched_count == total,
...
def make_fidelis_args(args, max_length, top_k):
    return SimpleNamespace(
        ...
        skip_final_reasoning=args.benchmark == "CR-LT-KGQA",
    )
...
def is_notknown_prediction(prediction):
    return normalize(str(prediction)) == "notknown"


def prediction_supported_by_paths(prediction, reasoning_paths):
    prediction_items = [
        item
        for item in split_predictions(prediction)
        if not is_notknown_prediction(item)
    ]
    if not prediction_items:
        return True

    path_text = normalize(" ".join(str(path) for path in reasoning_paths or []))
    return all(normalize(item) in path_text for item in prediction_items)


def apply_path_support_guard(metrics, prediction, reasoning_paths, benchmark):
    if benchmark not in {"RoG-webqsp", "RoG-cwq"}:
        return metrics
    if prediction_supported_by_paths(prediction, reasoning_paths):
        metrics["path_supported"] = True
        return metrics

    guarded = copy.deepcopy(metrics)
    guarded["acc"] = 0.0
    guarded["hit"] = 0
    guarded["f1"] = 0
    guarded["precision"] = 0.0
    guarded["recall"] = 0.0
    guarded["path_supported"] = False
    guarded["path_support_guard_applied"] = True
    return guarded
...
def run_one_config(sample, args, config, navigators):
    ...
    llm_metrics = score_for_benchmark(prediction_llm, res["ground_truth"], args.benchmark)
    if args.require_path_support:
        llm_metrics = apply_path_support_guard(
            llm_metrics,
            prediction_llm,
            reasoning_path,
            args.benchmark,
    ...
    direct_metrics = score_for_benchmark(
        prediction_direct_answer, res["ground_truth"], args.benchmark
    )
    is_correct = llm_metrics["f1"] == 1.0
    if args.benchmark == "CR-LT-KGQA":
        coverage = score_crlt_path_coverage(
            reasoning_path,
            sample.get("gold_kg_triples", []),
        )
        is_correct = coverage["full_coverage"]
    ...
    return {
        ...
        "prediction_direct_answer": prediction_direct_answer,
...
def default_output_label_file(args):
    return os.path.join(args.output_dir, f"{args.benchmark}_train_router_labels.jsonl")


def default_runtime_error_file(args):
    return os.path.join(args.output_dir, f"{args.benchmark}_train_router_runtime_errors.jsonl")


def default_skipped_ids_file(args):
    return os.path.join(args.output_dir, f"{args.benchmark}_train_router_skipped_ids.jsonl")


def resolve_output_paths(args):
    if args.output_label_file is None:
        args.output_label_file = default_output_label_file(args)
    if args.output_file:
    ...
    if args.runtime_error_file is None:
        args.runtime_error_file = default_runtime_error_file(args)
    if args.skipped_ids_file is None:
        args.skipped_ids_file = default_skipped_ids_file(args)
...
def label_samples(args):
    ...
    crlt_gold_triples = (
        load_crlt_qa_gold_triples(args.crlt_data_dir)
        if args.benchmark == "CR-LT-KGQA"
        else {}
    )
    ...
    for sample_idx in sample_indices:
        ...
        if args.max_candidates is not None and scanned_count >= args.max_candidates:
            break
        sample = dict(dataset[sample_idx])
        sample_id = sample.get("id")
        if sample_id in skip_ids or sample_id in seen_ids or sample_id in skipped_ids:
            continue
        if args.benchmark == "CR-LT-KGQA":
            sample["gold_kg_triples"] = crlt_gold_triples.get(sample_id, [])
        ...
        for config in configs:
            ...
            if attempt["is_correct"]:
                ...
                if (
                    args.benchmark != "CR-LT-KGQA"
                    and config["max_length"] == 1
                    and config["top_k"] == 1
                ):
        ...
        if label is None and correct_attempts:
            best_attempt = min(
                correct_attempts,
                key=lambda item: (
                    item["config"]["cost"],
            ...
            label = copy.deepcopy(best_attempt["config"])
            status = "resolved"
...
def parse_args():
    ...
    parser.add_argument("--runtime_error_file", type=str, default=None)
    parser.add_argument("--skipped_ids_file", type=str, default=None)
