...
def label_samples(args):
    ...
    for sample_idx in sample_indices:
        ...
        weak_label = None
        if status == "unresolved" and args.weak_label_unresolved:
            weak_label = select_weak_label(
                attempts,
                min_f1=args.weak_min_f1,
                min_margin=args.weak_min_margin,
            )
            if weak_label:
                label = weak_label["label"]
                status = "weak_resolved"
...
def parse_args():
    ...
    parser.add_argument(
        "--weak_label_unresolved",
        action="store_true",
        help="Promote unresolved samples to weak_resolved when one config is clearly best by F1.",
    )
    parser.add_argument("--weak_min_f1", type=float, default=0.5)
    parser.add_argument("--weak_min_margin", type=float, default=0.25)
